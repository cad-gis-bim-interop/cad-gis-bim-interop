...
- **GIS formats** (GeoJSON, Shapefile, GPKG) typically embed WKT strings or EPSG identifiers directly in headers or sidecar files (`.prj`).
...
def detect_crs_from_metadata(source_path: str, format_type: str) -> Optional[CRS]:
    ...
    if format_type == "gis":
        ...
        return gdf.crs
    elif format_type in ("cad", "bim"):
        # DXF/IFC parsing requires external control file or manual EPSG assignment
        return None
    return None
```
...
def validate_and_resolve_crs(raw_crs: Optional[str], fallback: str = "EPSG:4326") -> CRS:
    """Validate CRS string and resolve to a pyproj CRS object."""
    if raw_crs is None:
        print(f"[WARN] No CRS detected. Applying fallback: {fallback}")
        return CRS.from_epsg(int(fallback.split(":")[1]))

    try:
        # from_user_input accepts EPSG codes, WKT, PROJ strings, and authority:code
        resolved = CRS.from_user_input(raw_crs)
        return resolved
    except Exception as e:
        print(f"[ERROR] CRS resolution failed: {e}. Falling back to {fallback}")
...
During this phase, monitor for vertical datum mismatches. If your pipeline ingests LiDAR point clouds or BIM elevation data alongside 2D GIS vectors, ensure the target CRS includes a vertical component (e.g., `EPSG:7844` or a compound CRS definition). Misaligned vertical datums frequently cause clash detection false positives in infrastructure modeling.
...
1. **Chunked Processing:** For datasets exceeding available RAM, use `pyogrio` streaming or `geopandas` row filtering to process features in batches.
