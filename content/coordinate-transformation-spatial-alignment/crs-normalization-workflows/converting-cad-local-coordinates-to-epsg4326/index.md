...
* **Axis Order Enforcement:** Modern `pyproj` (v2.0+) strictly follows CRS axis definitions. EPSG:4326 expects `(latitude, longitude)`, while CAD uses `(X, Y)` ≈ `(eastings, northings)`. Initialize transformers with `always_xy=True` to prevent silent swaps.
