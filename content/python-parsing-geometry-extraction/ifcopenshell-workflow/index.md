...
4. **Async Batch Processing**: Combine `asyncio` with `concurrent.futures.ProcessPoolExecutor` to maximize I/O throughput while keeping CPU-bound geometry tasks isolated in separate OS processes. Use a bounded queue (e.g., `asyncio.Queue(maxsize=N)`) for backpressure so that ingestion pauses when geometry workers are saturated.
