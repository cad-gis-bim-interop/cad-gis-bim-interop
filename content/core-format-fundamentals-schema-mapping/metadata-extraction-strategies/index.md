...
- **Core Libraries**: `ezdxf` (DXF parsing), `ifcopenshell` (IFC schema traversal), `geopandas` + `fiona` (GIS vector handling), `pydantic` (schema validation), `lxml` (XML fallback parsing)
...
Never rely solely on file extensions, which are frequently mislabeled in enterprise data lakes or legacy FTP drops. Implement a lightweight header inspection routine that reads magic bytes or initial ASCII markers. IFC files typically begin with `ISO-10303-21;`, while DXF files start with `0\nSECTION`. GeoPackage files use a SQLite header identifiable via standard file signatures.
...
Open files using context managers to guarantee resource cleanup, especially when processing hundreds of files concurrently. Note that `ifcopenshell.open()` returns a plain `ifcopenshell.file` object — it does not implement the context manager protocol and cannot be used with `with` directly.
...
def open_ifc_file(filepath: str) -> ifcopenshell.file:
    """Open an IFC file and return the parsed model. Caller is responsible for cleanup."""
    try:
        return ifcopenshell.open(filepath)
    except Exception as e:
        raise RuntimeError(f"IFC parser initialization failed for {filepath}: {e}")

def open_dxf_file(filepath: str) -> ezdxf.document.Drawing:
    """Open a DXF file. Use doc.close() when done to release resources."""
    try:
        return ezdxf.readfile(filepath)
    except ezdxf.DXFError as e:
        raise RuntimeError(f"DXF parser initialization failed for {filepath}: {e}")
```

Initialize schema-aware readers that expose attribute dictionaries without loading full geometry into memory. Memory mapping is critical for BIM models exceeding 500MB, where naive loading triggers `MemoryError` exceptions.
...
- **Schema Versioning**: Tag extracted datasets with the parser version and source schema revision (e.g., `ifcopenshell==0.8.0`, `schema=IFC4x3`). This enables backward compatibility checks when upstream vendors upgrade their export tools.
- **Fallback Routing**: When a primary parser fails (e.g., an IFC file contains non-standard content), route the file to a secondary extraction path using `lxml` for direct XML traversal. Document fallback thresholds clearly to prevent silent data loss.
