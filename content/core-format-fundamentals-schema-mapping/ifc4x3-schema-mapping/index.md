...
def traverse_linear_entities(model: ifcopenshell.file) -> Generator[Dict, None, None]:
    ...
    targets = (
        model.by_type("IfcAlignment")
        + model.by_type("IfcBridge")
        + model.by_type("IfcRailway")
    )
...
def transform_geometry_to_geojson(
    ...
    for entity in model.by_type("IfcAlignment"):
        try:
            ...
            # shape_obj.geometry contains verts/faces; build GeoJSON manually in production
            if shape_obj:
                geo_features.append({
                    "type": "Feature",
                    "properties": {"guid": entity.GlobalId, "type": entity.is_a()},
                    "geometry": None  # Replace with coordinate extraction from shape_obj
                })
        except Exception as e:
...
1. **Header Pre-Checks:** Read the first 50 lines of the `.ifc` file as raw text to verify `FILE_SCHEMA` before invoking `ifcopenshell.open()`. This avoids costly memory allocation on incompatible files.
...
5. **Geometry Tolerance Control:** STEP representations contain high-precision NURBS curves. Apply tolerance settings in `ifcopenshell.geom` to reduce vertex count before GeoJSON export. When only metadata mapping is required, avoid geometry parsing entirely: open the file normally and skip calls to `ifcopenshell.geom.create_shape()`.
